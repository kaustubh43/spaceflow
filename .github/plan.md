...
- **Dark‑mode canvas + 3D**: dark floor sheet/backdrop + lifted (light) wall/structural colours and grid; furniture colours preserved; 3D view follows the theme with walls kept light. The dark palette is a blue‑rich **navy** ramp (custom `navy` Tailwind scale) with clear value steps between bg → surface → raised → border, replacing the near‑black flat slate; 2D/3D grids and the floor sheet use matching navy tints.
